...
public class WitherGun implements Listener {
    ...
    public void onLaunch(ProjectileLaunchEvent e) {
        ...
        if (entity instanceof Snowball) {
            ...
            snowball.setMetadata("withergun", new FixedMetadataValue(rAbilities.getInstance(), player.getUniqueId()));
    ...
    public void onClick(PlayerInteractEvent event) {
        ...
        if (Cooldowns.getWithergun().onCooldown(player)) {
            for (String list : rAbilities.config.getConfig().getStringList("Message.Item.OnCooldown")) {
                player.sendMessage(chatUtil.chat(list.replace("%ability%", rAbilities.config.getConfig().getString("WitherGun.Name")).replace("%time%", Cooldowns.getWithergun().getRemaining(player))));
                event.setCancelled(true);
                player.updateInventory();
                return;
            }
        }
        Cooldowns.getWithergun().applyCooldown(player, rAbilities.config.getConfig().getInt("WitherGun.Cooldown") * 1000);
        ...
        if (e.getEntity() instanceof Player) {
            if (e.getDamager() instanceof Snowball) {
                ...
                if (snowball.getShooter() instanceof Player) {
                    damaged.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, rAbilities.config.getConfig().getInt("WitherGun.Effect.Time") * 23,
                            rAbilities.config.getConfig().getInt("WitherGun.Effect.Power")));
                    for (String list : rAbilities.config.getConfig().getStringList("WitherGun.Message.Been-Hit")) {
                        damaged.sendMessage(chatUtil.chat(list).replaceAll("%player%", ((Player) snowball.getShooter()).getName()));
                    }
                    for (String list : rAbilities.config.getConfig().getStringList("WitherGun.Message.Hit-Someone")) {
